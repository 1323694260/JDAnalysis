...
def running_game(screen,snake_speed_clock):
	...
	start_img1 = random.randint(1,12) # 开始图片
	start_img2 = random.randint(1,12) # 开始图片
	start_img3 = random.randint(1,12) # 开始图片
	snake_coords = [{'x': startx, 'y': starty, 'img': start_img1},  # 初始贪吃蛇
                  {'x': startx - 1, 'y': starty, 'img': start_img2},
                  {'x': startx - 2, 'y': starty, 'img': start_img3}]
...
def draw_food(screen, food):
	...
	# appleRect = screen.blit(pygame.image.load('1.png'),(x,y))
	screen.blit(pygame.image.load('img/'+str(food['img'])+'.png'), (x, y, cell_size, cell_size)) # 将食物设置成图片
	# pygame.draw.rect(screen, (255,255,255,0.6), appleRect)
#将贪吃蛇画出来
def draw_snake(screen, snake_coords):
	for coord in snake_coords:
		...
		screen.blit(pygame.image.load('img/'+str(img)+'.png'),(x, y, cell_size, cell_size))
...
def move_snake(direction, snake_coords):
    ...
    if direction == UP:
        newHead = {'x': snake_coords[HEAD]['x'], 'y': snake_coords[HEAD]['y'] - 1, 'img': snake_coords[-1]['img']}
    elif direction == DOWN:
        newHead = {'x': snake_coords[HEAD]['x'], 'y': snake_coords[HEAD]['y'] + 1, 'img': snake_coords[-1]['img']}
    elif direction == LEFT:
        newHead = {'x': snake_coords[HEAD]['x'] - 1, 'y': snake_coords[HEAD]['y'], 'img': snake_coords[-1]['img']}
...
def snake_is_eat_food(snake_coords, food):  #如果是列表或字典，那么函数内修改参数内容，就会影响到函数体外的对象。
	if snake_coords[HEAD]['x'] == food['x'] and snake_coords[HEAD]['y'] == food['y']:
		snake_coords[HEAD]['img'] = food['img']    # 吃到的食物添加到蛇头
		food['x'] = random.randint(0, map_width - 1)
		food['y'] = random.randint(0, map_height - 1) # 实物位置重新设置
		food['img'] = random.randint(1, 12) # 图片更换

	else:
		del snake_coords[-1]  # 如果没有吃到实物, 就向前移动, 那么尾部一格删掉
#食物随机生成
def get_random_location():
	return {'x': random.randint(0, map_width - 1), 'y': random.randint(0, map_height - 1), 'img': random.randint(1, 12)}
#开始信息显示
